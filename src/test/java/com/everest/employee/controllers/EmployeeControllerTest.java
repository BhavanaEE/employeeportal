...
class EmployeeControllerTest {
    ...
    void shouldUpdateEmployee() throws Exception {
        Long employeeId=2L;
        given(employeeService.updateEmployee(eq(employeeId),any(Employee.class))).willReturn(employee);
        this.mockMvc.perform(put("/api/employees/{id}",employeeId)
    ...
    void shouldReturn404WhenUpdatingNonExistingEmployee() throws Exception {
        ...
        this.mockMvc.perform(put("/api/users/{id}", employeeId)
                        .content(objectMapper.writeValueAsString(employee)))
